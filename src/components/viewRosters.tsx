...
function ViewRosters(): JSX.Element  {
  ...
  const handleRosterSelect = (id: number) => {
    const selectedRoster: RosterDataItem | undefined = displayRosters(rosters)
    .find((roster) => id === roster.id);
    if (selectedRoster) {
      setExpSelected({
  ...
  return (
    <div className="m-2">
    <ListContainer items={displayRosters(rosters)}
        listItem={RosterListItem}
        title="Rosters"
    >
