...
function ViewRoster() {
  const { selected, setExpSelected } = useExperienceStore(
    useShallow((state) => ({
  ...
  const {
    ...
    opt_in = [],
    opt_out = [],
  } = (selectedRoster as RosterDataItem) || {};
  ...
  const unUsedPlayers = players?.filter((player: DataItem) => {
    return (
      !selectedRoster?.allow?.includes(player.snowflake as string) &&
      ...
      >
        <div className={`min-w-[46vw] bg-slate-600 shrink`}>
          ...
          <div className="flex flex-wrap gap-2 p-1 mb-4">
            {allow &&
              allow.map((playerSnowflake: string) => (
                <Chip
            ...
            {allow.length == 0 && (
              <p className="text-slate-400 p-1 text-lg">All non-banned players allowed.</p>
            )}
            <SelectChip
              selectPlaceholder="Add Player"
              selections={
                players
                  ? unUsedPlayers.map((item: DataItem) => ({
                      label: item.name,
          ...
          <div className="flex flex-wrap gap-2 p-1 mb-4">
            {deny &&
              deny.map((playerSnowflake: string) => (
                <Chip
