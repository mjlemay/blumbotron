...
function DisplayListItem(props: ComponentProps): JSX.Element {
  ...
  const getLimitedPlayers = (players: string[], allPlayers: DataItem[]): string => {
    ...
    if (playerLength > PLAYER_LIMIT) {
      ...
      const namedPlayers = firstPlayers.map(
        (player: string) => getPlayerBySnowflake(player, allPlayers)?.name || player
      );
      return namedPlayers.join(', ') + ' and ' + (playerLength - PLAYER_LIMIT) + ' more...';
    }
    return players
      .map((player: string) => getPlayerBySnowflake(player, allPlayers)?.name || player)
      .join(', ');
  };

  return (
    <div
      className="flex flex-col items-start justify-start bg-slate-700 hover:cursor-pointer hover:bg-blue-600/20 rounded-lg shadow-lg p-4 m-2"
      key={`${id}_${name}`}
